<x-drawer wire:model="showDrawer" :title="$form->isEditMode ? __('Edit Employee Profile') : __('Complete Employee Profile')" right class="w-full sm:w-[600px] lg:w-2/3 max-w-full" separator with-close-button>
    <x-form wire:submit="save">
                ...
                <x-slot:content>
                    <div class="space-y-6 p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {{-- Position --}}
                            <x-choices-offline label="{{ __('Position') }}" :options="$positions" wire:model="form.position_id"
                                icon="mdi.briefcase" placeholder="{{ __('Select position') }}" single searchable required />
                            ...
                            {{-- Manager --}}
                            <x-choices-offline label="{{ __('Manager') }}" :options="$managers" wire:model="form.manager_id"
                                icon="mdi.account-supervisor" placeholder="{{ __('Select manager') }}" single searchable />

                            {{-- Contract Type --}}
                            <x-choices-offline label="{{ __('Contract Type') }}" :options="$contractTypes" wire:model="form.type"
                                icon="mdi.file-document" placeholder="{{ __('Select contract type') }}" single searchable required />
                            ...
                            {{-- Compensation Unit --}}
                            <x-choices-offline label="{{ __('Compensation Unit') }}" :options="$compensationUnits" wire:model="form.compensation_unit"
                                icon="mdi.calendar-clock" placeholder="{{ __('Select compensation unit') }}" single searchable required />
                            ...
                            <div class="md:col-span-2">
                                <x-file wire:model="form.contract_file" accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            ...
            <x-collapse id="bank-details" wire:model="showBankDetails" class="bg-base-200">
                ...
                <x-slot:content>
                    <div class="space-y-6 p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {{-- Bank Name --}}
                            <x-input label="{{ __('Bank Name') }}" wire:model="form.bank_name"
                                icon="mdi.bank" placeholder="{{ __('Bank name') }}" />
        ...
        <x-slot:actions>
            <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center w-full gap-3">
                ...
                <x-button
                    :label="$form->isEditMode ? __('Update') : __('Complete')"
                    type="submit"
                    spinner="save"
                    class="btn-primary"
                />
