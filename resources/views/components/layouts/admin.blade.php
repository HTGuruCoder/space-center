    ...
    <body>
        ...
        <header class="navbar bg-base-100 shadow-md sticky top-0 z-40">
            <div class="navbar-start">
                {{-- Logo --}}
                <a href="{{ route('admins.dashboard') }}" class="btn btn-ghost text-xl">
                    <x-icon name="mdi.shield-crown" class="w-6 h-6" />
                    <span class="hidden sm:inline">SpaceCenter Admin</span>
                </a>
            </div>
        ...
        <footer class="footer footer-center p-4 bg-base-300 text-base-content mt-auto">
            <div>
                <p>{{ __('Copyright © :year - SpaceCenter Admin', ['year' => date('Y')]) }}</p>
            </div>
        </footer>
